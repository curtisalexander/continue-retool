...
def sync(*, check: bool, packaging_only: bool = False) -> bool:
    ...
    gateway_count = metrics["gateway"]["tool_count"]
    ...
    gateway_cost = (
        f"The audit currently estimates the gateway's {gateway_count} meta-tool schemas at "
    ...
    token_math = (
        ...
        f"if registered directly. The gateway advertises {gateway_count} meta-tools totaling **~{gateway} "
    ...
    gateway_overview = (
        ...
        f"The gateway's fixed advertisement is **{gateway_count} tools totaling "
        f"~{gateway} tokens**. "
