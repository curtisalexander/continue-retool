...
> Measure the schemas you actually have. Even the current two-server tail beats
> the gateway's fixed cost; a very small-schema tail may not.
...
```bash
# From continue-mcp/. Here sql and notes are the tail hidden by the gateway.
uv run install-workspace.py /path/to/workspace --gateway --only sql,notes

# Verify the exact installed gateway and its downstream catalog.
uv run install-workspace.py /path/to/workspace --gateway --only sql,notes --check
```

The installer stamps absolute `uv` and package paths, `--no-sync`,
`MCP_WORKSPACE`, and a project-owned `.continue/gateway.config.json`. Use
`--gateway --uninstall` with the same `--only` selection to restore the original
workspace configuration. For development, `uv run --project . gateway-mcp`
from the toolkit root starts the checkout's example config directly.

### Wire it into Continue — and register ONLY the gateway

1. Install with `install-workspace.py --gateway`; it writes the stamped
   `.continue/mcpServers/gateway.yaml` for you.
2. **Do NOT also add downstream yamls to Continue.** The
...
- **In-process variant:** instead of spawning downstream servers, the gateway could
  ...
  loses process isolation and changes lifecycle behavior. The aggregator design
  keeps each server independently registerable even though packaging is unified.
...
- **Partial startup:** one unavailable downstream is reported in
  `search().unavailable_servers` while healthy servers remain usable. The installer
  doctor treats any such degraded connection as a failed check, so a broken setup
  cannot pass validation silently.
