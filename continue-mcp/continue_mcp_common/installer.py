...
from continue_mcp_common.atomic import create_from_temp  # noqa: E402
...
def detect_shell_env() -> dict[str, str]:
    ...
    detected["SHELL_MCP_PREFERRED_SHELL"] = default
    return detected


def _source_checkout() -> bool:
    return (KIT_DIR / "pyproject.toml").is_file() and (KIT_DIR / "uv.lock").is_file()


def _launch(name: str, uv: str) -> tuple[str, list[str]]:
    if _source_checkout():
        return os.path.abspath(uv), ["run", "--no-sync", "--project", str(KIT_DIR), f"{name}-mcp"]
    return sys.executable, ["-m", f"{name}_mcp.server"]


def render_config(name: str, uv: str, workspace: Path, shell_env: dict[str, str] | None = None) -> str:
    command, args = _launch(name, uv)
    ...
    try:
        for path, text in outputs.items():
            ...
            if prior is not None:
                ...
                create_from_temp(temporary, path)
...
async def _handshake(
    name: str, uv: str, workspace: Path, env: dict[str, str]
) -> str | None:
            ...
            with tempfile.TemporaryDirectory(prefix=".continue-mcp-check-", dir=workspace) as temporary:
                ...
                if name == "fs":
                    ...
                    if shell in ("pwsh", "powershell"):
                        unicode_marker = "雪🚀"
                        command = (
                            "$edition = if ($PSVersionTable.PSEdition) "
                            "{$PSVersionTable.PSEdition} else {'Desktop'}; "
                            "Write-Output "
                            f"('{marker}' + [char]9 + $PSVersionTable.PSVersion.ToString() + "
                            f"[char]9 + $edition + [char]9 + '{unicode_marker}')"
                        )
                        result = await client.call_tool(
                            "run",
                            {"cmd": command, "shell": shell, "timeout": 15},
                            raise_on_error=False,
                        )
                        diagnostic = _assert_result(name, result, unicode_marker)
                        fields = diagnostic.get("stdout", "").strip().split("\t")
                        if (
                            diagnostic.get("exit_code") != 0
                            or len(fields) != 4
                            or fields[0] != marker
                            or fields[3] != unicode_marker
                        ):
                            raise RuntimeError(
                                "shell PowerShell capability check returned invalid diagnostics"
                            )
                        actual_shell = diagnostic.get("shell")
                        executable = diagnostic.get("interpreter")
                        if actual_shell != shell or not executable:
                            raise RuntimeError(
                                "shell PowerShell capability check omitted selected interpreter"
                            )
                        forced = env.get("SHELL_MCP_DEFAULT_SHELL")
                        preferred = env.get("SHELL_MCP_PREFERRED_SHELL")
                        if forced:
                            selection = f"explicit default {forced}"
                        elif preferred == actual_shell:
                            selection = f"preferred {preferred}"
                        elif preferred:
                            selection = f"preferred {preferred} unavailable; fallback"
                        else:
                            selection = "platform fallback"
                        return (
                            f"PowerShell: executable={executable} shell={actual_shell} "
                            f"version={fields[1]} edition={fields[2]} "
                            f"unicode=ok selection={selection}"
                        )
                elif name == "edit":
...
def check(project: str, selected: list[str], uv: str) -> None:
    ...
    for name in selected:
        ...
        diagnostic = asyncio.run(
            _handshake(name, os.path.abspath(uv), workspace, _config_env(outputs[path]))
        )
        print(f"ok {name}-mcp")
        if diagnostic:
            print(f"  {diagnostic}")
...
def main() -> int:
    # Redirected Windows Python 3.11 streams may use an ANSI code page. Logging
    # a legal Unicode path must not roll back an otherwise successful install.
    ...
    try:
        ...
        if _source_checkout() and not shutil.which("uv"):
        ...
        if args.check:
            ...
            if not args.no_sync and _source_checkout():
                sync_deps(uv)
