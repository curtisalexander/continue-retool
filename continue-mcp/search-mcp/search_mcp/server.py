...
def rg_bin() -> str:
    ...
    if not b:
        raise RuntimeError(
            ...
            "  4. Set RIPGREP_BIN in the server YAML to an absolute executable path.\n"
            "     PowerShell session: $env:RIPGREP_BIN = 'C:\\Tools\\rg.exe'\n"
            "     Bash session: export RIPGREP_BIN=/abs/path/to/rg\n"
            "     GUI-launched servers may not inherit terminal environment changes.\n"
